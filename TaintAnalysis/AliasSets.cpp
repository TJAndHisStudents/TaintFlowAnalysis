...
int AliasSets::getValueSetKey(Value* v) {
    ...
    if (valueDisjointSet.count(v))
    {
     //   errs()<<"\n\nValue  found in Alias set..!! "<< valueDisjointSet.count(v) <<"  "<<valueDisjointSet[v];

        return valueDisjointSet[v];
    }

	//        assert(0 && "Value requested is not in any alias set!");
 //   errs()<<"\n\nValue not found in the ALias Set..!!! ";
	return 0;
