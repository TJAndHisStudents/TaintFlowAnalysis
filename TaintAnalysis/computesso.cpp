...
#include "computesso.h"
//#include "TaintFlow.h"
//#define DEBUG_TYPE "ComputeSSO::"
...
void ComputeSSO::updateTaintLabels()
{
...
void ComputeSSO::propagateLabel(GraphNode* snode)
{
...
void ComputeSSO::getSinkSourceBlockDependence(){
...
void ComputeSSO::getHookLocation(){
    errs()<<"\n***************************** Hooks Identified ***********************************\n";
...
bool ComputeSSO::isValueTainted(Value* v) {
    GraphNode* g = depGraph->findNode(v);
    return tainted.count(g);
}

std::set<GraphNode*> ComputeSSO::getTaintedValues() {
    return tainted;
}

void ComputeSSO::getAnalysisUsage(AnalysisUsage &AU) const {
    AU.setPreservesAll();
...
char ComputeSSO::ID = 0;
static RegisterPass<ComputeSSO> S("getSSO", "Compute the security sensitive objects and operations.");
